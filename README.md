...
- スクリーンショットの自動ファイル保存 — REST API (`POST /screenshot`, `POST /goal`) 経由のキャプチャもダッシュボードに表示
- ダッシュボードのスクリーンショット表示を縦長 (フルページ) 画像に対応
- Type/Scroll/Hover/Select アクションで要素の存在チェックを追加 (null 参照バグ修正)
- ダッシュボードのスクリーンショット一覧を撮影日時順にソート

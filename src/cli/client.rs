...
impl WbClient {
        ...
        #[cfg(unix)]
        {
            ...
            if let Ok(meta) = std::fs::metadata(&token_path) {
                if meta.mode() & 0o077 != 0 {
                    eprintln!(
                        "Warning: {} is accessible by other users. Run: chmod 600 {}",
                        token_path.display(),
                        token_path.display()
                    );
                }
    ...
    fn map_ureq_error(e: ureq::Error) -> WbError {
        match e {
            ureq::Error::Status(401, _) | ureq::Error::Status(403, _) => WbError::auth(
            ...
            ureq::Error::Status(code, resp) => {
                ...
                if msg.contains("SESSION_NOT_FOUND") {
                    ...
                    WbError::session(
                        "Session exists but is not acquired. Run: wb session acquire <name>",
                    )
                } else if msg.contains("SESSION_LIMIT") {
                    WbError::general(
                        "Maximum session limit reached. Release unused sessions: wb session release <name>",
                    )
                } else {
            ...
            ureq::Error::Transport(t) => {
                ...
                if detail.contains("timed out") {
                    WbError::timeout(
                        "Request timed out. The server may be overloaded. Try again or increase timeout.",
                    )
                } else {

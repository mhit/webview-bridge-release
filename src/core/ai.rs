...
"#;
        ...
        serde_json::from_str(&json_str).map_err(|e| {
            format!(
                "Failed to parse login analysis: {} - Response: {}",
                e, response
            )
        })
    }
    ...
    pub fn analyze_image(
        ...
        let parsed: serde_json::Value = serde_json::from_str(&json_str).map_err(|e| {
            format!(
                "Failed to parse image analysis: {} - Response: {}",
                e, response
            )
        })?;
    ...
    pub fn extract_from_screenshot(
        &self,
...
"#
    ...
    pub fn generate_image_analysis_prompt(
        analysis_type: &ImageAnalysisType,
        custom_criteria: &[String],
    ) -> String {
...
"#, criteria_str)
    ...
    pub fn generate_extract_prompt(
        description: &str,
        schema: Option<&serde_json::Value>,
    ) -> String {
...
mod tests {
    ...
    fn test_gemini_prompts() {
        ...
        let image_prompt =
            GeminiClient::generate_image_analysis_prompt(&ImageAnalysisType::ProductQuality, &[]);
    ...
    fn test_gemini_prompt_composition() {
        let prompt =
            GeminiClient::generate_image_analysis_prompt(&ImageAnalysisType::Composition, &[]);
    ...
    fn test_gemini_prompt_brand() {
        let prompt =
            GeminiClient::generate_image_analysis_prompt(&ImageAnalysisType::BrandConsistency, &[]);
    ...
    fn test_gemini_prompt_custom() {
        ...
        let prompt =
            GeminiClient::generate_image_analysis_prompt(&ImageAnalysisType::Custom, &criteria);
    ...
    fn test_gemini_extract_prompt() {
        let prompt = GeminiClient::generate_extract_prompt("商品名と価格", None);
    ...
    fn test_gemini_extract_prompt_with_schema() {
        ...
        let prompt = GeminiClient::generate_extract_prompt("商品情報", Some(&schema));

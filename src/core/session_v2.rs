...
fn default_auto_restore() -> bool {
    true
} // Auto-restore last URL by default
...
impl SessionManagerV2 {
    ...
    fn load_sessions(&self) -> Result<(), String> {
        ...
        println!(
            "[SessionManagerV2] Loaded {} sessions from disk",
            sessions.len()
        );
        Ok(())
    }
    ...
    fn cleanup_orphaned_profiles(&self) {
        ...
        let session_profiles: std::collections::HashSet<String> =
            sessions.values().map(|s| s.meta.profile.clone()).collect();
        ...
        if let Ok(entries) = fs::read_dir(&profiles_dir) {
                    ...
                    if meta.is_dir() {
                        ...
                        if !session_profiles.contains(&dir_name) {
                            if let Err(e) = fs::remove_dir_all(entry.path()) {
                                eprintln!(
                                    "[SessionManagerV2] Warning: Failed to remove orphan profile {:?}: {}",
                                    dir_name, e
                                );
                            } else {
                                orphan_count += 1;
            ...
            if orphan_count > 0 {
                println!(
                    "[SessionManagerV2] Cleaned up {} orphaned profile directories",
                    orphan_count
                );
            }
        }
    ...
    pub async fn acquire(
        ...
        if !request.create_if_missing {
            return Err(format!(
                "Session '{}' not found and create_if_missing is false",
                request.name
            ));
        }
        ...
        // Check max sessions
        {
            ...
            let active_count = sessions.values().filter(|s| s.handle.is_some()).count();
            ...
            for (_name, session) in sessions.iter_mut() {
                if let Some(ref handle) = session.handle {
                    if handle.id == session_id {
                        tracing::info!(
                            "[SessionV2] Window closed by user, clearing handle for session with id={}",
                            session_id
                        );
                        session.handle = None;
                        session.acquired = false;
    ...
    pub fn suspend_session(&self, name: &str) -> Result<Option<String>, String> {
        ...
        tracing::info!(
            "[SessionManagerV2] Suspended session '{}', handle={:?}",
            name,
            handle_id
        );
    ...
    pub fn auto_suspend_idle(&self, idle_seconds: u64) -> Vec<(String, String)> {
        ...
        if !suspended.is_empty() {
            tracing::info!(
                "[SessionManagerV2] Auto-suspended {} idle sessions: {:?}",
                suspended.len(),
                suspended
                    .iter()
                    .map(|(n, _)| n.as_str())
    ...
    pub fn get_auto_login_op_item_id(&self, name: &str) -> Option<String> {
        ...
        sessions
            .get(name)
            .and_then(|s| s.meta.auto_login_op_item_id.clone())
    }
    ...
    pub fn record_auto_login_success(
        &self,
        name: &str,
        op_item_id: &str,
        username: &str,
        ...
        session.meta.auto_login_attempt_count =
            session.meta.auto_login_attempt_count.saturating_add(1);
    ...
    pub fn record_auto_login_failure(&self, name: &str, error: &str) -> Result<(), String> {
        ...
        session.meta.auto_login_attempt_count =
            session.meta.auto_login_attempt_count.saturating_add(1);
        session.meta.auto_login_consecutive_failures = session
            .meta
            .auto_login_consecutive_failures
            .saturating_add(1);
        drop(sessions);
        self.save_sessions()
    }
        ...
        std::thread::spawn(move || {
            ...
            for attempt in 0..5 {
                ...
                match fs::remove_dir_all(&dir) {
                    Ok(_) => {
                        println!(
                            "[SessionManagerV2] Deleted profile directory: {}",
                            profile_owned
                        );
                        return;
                    }
                    Err(e) if attempt < 4 => {
                        eprintln!(
                            "[SessionManagerV2] Retry {}/4 deleting {:?}: {}",
                            attempt + 1,
                            dir,
                            e
                        );
                    ...
                    Err(e) => {
                        eprintln!(
                            "[SessionManagerV2] Failed to delete profile dir {:?} after 5 attempts: {}",
                            dir, e
                        );
                    }
                }
    ...
    pub fn cleanup_inactive(&self) -> Result<Vec<String>, String> {
        ...
        println!(
            "[SessionManagerV2] Cleaned up {} inactive sessions",
            to_remove.len()
        );
    ...
    pub fn cleanup_old(&self, max_age_hours: u64) -> Result<Vec<String>, String> {
        ...
        for (name, session) in sessions.iter() {
            if let Ok(ts) = chrono::DateTime::parse_from_rfc3339(&session.meta.last_accessed) {
                let session_time =
                    SystemTime::UNIX_EPOCH + Duration::from_secs(ts.timestamp() as u64);
        ...
        println!(
            "[SessionManagerV2] Cleaned up {} old sessions (older than {}h)",
            to_remove.len(),
            max_age_hours
        );
    ...
    pub fn cleanup_expired(&self) -> Result<Vec<String>, String> {
        ...
        if !to_remove.is_empty() {
            println!(
                "[SessionManagerV2] Cleaned up {} expired sessions",
                to_remove.len()
            );
        }
    ...
    pub fn cleanup_idle(&self, max_idle_seconds: u64) -> Vec<String> {
        ...
        if !removed.is_empty() {
            tracing::info!(
                "[SessionManagerV2] Cleaned up {} idle sessions: {:?}",
                removed.len(),
                removed
            );
        }
...
mod tests {
    ...
    fn test_session_manager_with_idle_timeout() {
        ...
        let _manager = SessionManagerV2::new(temp_dir.clone(), 10).with_idle_timeout(3600);

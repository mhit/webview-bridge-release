...
pub fn analyze_with_llm(
    ...
    let needs_analysis: Vec<&serde_json::Value> = elements.iter()
        .filter(|el| {
...
"#);
    ...
    for el in elements {
        ...
        let cursor = visual.and_then(|v| v.get("cursor")).and_then(|c| c.as_str()).unwrap_or("");
        let has_transition = visual.and_then(|v| v.get("hasTransition")).and_then(|t| t.as_bool()).unwrap_or(false);
        let has_image = visual.and_then(|v| v.get("hasImage")).and_then(|i| i.as_bool()).unwrap_or(false);
        let bg = visual.and_then(|v| v.get("backgroundColor")).and_then(|b| b.as_str()).unwrap_or("");
        let rule_score = el.get("interactivity").and_then(|i| i.get("score")).and_then(|s| s.as_f64()).unwrap_or(0.0);
        
        prompt.push_str(&format!(
            "\n{}. <{}> label=\"{}\" cursor={} transition={} image={} bg=\"{}\" ruleScore={:.2}",
            index, tag, label.chars().take(30).collect::<String>(), 
            cursor, has_transition, has_image, 
            bg.chars().take(30).collect::<String>(), rule_score
        ));
    }

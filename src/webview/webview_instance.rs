    ...
    fn Invoke(
        ...
        unsafe {
            ...
            if rect.right == 0 && rect.bottom == 0 {
                log_webview_debug("ControllerHandler", "Client rect is empty, using default 1024x768");
                rect.right = 1024;
                rect.bottom = 768;
            }

            log_webview_debug("ControllerHandler", &format!("Bounds: {}x{}", rect.right, rect.bottom));
            ...
            match controller.SetIsVisible(true) {
                ...
                Err(e) => {
                    log_webview_error("ControllerHandler", &format!("SetIsVisible failed: {:?}", e));
                    return Err(e);
                }
            }
            ...
            let webview = match controller.CoreWebView2() {
                ...
                Err(e) => {
                    log_webview_error("ControllerHandler", &format!("CoreWebView2 failed: {:?}", e));
                    return Err(e);
                }
            };
            ...
            // Register events
            log_webview_debug("ControllerHandler", "Registering NavigationStarting event...");
            let _ = webview.add_NavigationStarting(
                &ICoreWebView2NavigationStartingEventHandler::from(NavigationStartingHandler {}),
                &mut Default::default(),
            );

            log_webview_debug("ControllerHandler", "Registering NavigationCompleted event...");
            let _ = webview.add_NavigationCompleted(
                &ICoreWebView2NavigationCompletedEventHandler::from(NavigationCompletedHandler { hwnd: self.hwnd }),
                &mut Default::default(),
            );
        ...
        log_webview_debug("ControllerHandler", &format!("Storing controller for hwnd_val={}", hwnd_val));
...
// Fire-and-forget handler for operations that don't need result tracking
// (e.g., click, type_text, press_key - these are one-way operations)
#[windows::core::implement(ICoreWebView2ExecuteScriptCompletedHandler)]
...
impl ICoreWebView2ExecuteScriptCompletedHandler_Impl for FireAndForgetHandler {
    fn Invoke(
        ...
        // Simply log and ignore the result - no notification needed
...
impl WebViewInstance {
    pub fn new(title: &str, visible: bool) -> WinResult<Self> {
        log_webview_start("WebViewInstance::new", &format!("title={}, visible={}", title, visible));

        INIT_COM.call_once(|| unsafe {
            log_webview_debug("WebViewInstance::new", "Initializing COM");
        ...
        log_webview_debug("WebViewInstance::new", &format!("Window created with hwnd={:#x}", hwnd.0));
    ...
    pub fn initialize(&mut self, user_data_folder: &str) -> WinResult<()> {
        log_webview_start("WebViewInstance::initialize", &format!("user_data_folder={}", user_data_folder));
        ...
        unsafe {
            ...
            log_webview_debug("WebViewInstance::initialize", "Setting WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS");
            std::env::set_var(
                "WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS",
                "--disable-gpu --no-sandbox",
            );

            log_webview_debug("WebViewInstance::initialize", "Calling CreateCoreWebView2EnvironmentWithOptions");
    ...
    pub fn claim_controller(&mut self) {
        ...
        log_webview_start("WebViewInstance::claim_controller", &format!("hwnd_val={}", hwnd_val));
        ...
        if let Some(c) = controller {
            ...
            log_webview_error("WebViewInstance::claim_controller", "No pending controller found");
        }
    }
    ...
    pub fn execute_script(&self, script: &str, request_id: String) -> Result<String, Error> {
        ...
        log_webview_start("WebViewInstance::execute_script",
            &format!("request_id={}, script={}", request_id, script_preview));

        if let Some(controller) = &self.controller {
            unsafe {
                let webview = controller.CoreWebView2()?;
                log_webview_debug("WebViewInstance::execute_script", "Calling webview.ExecuteScript()");
    ...
    pub fn type_text(&self, selector: &str, text: &str) -> WinResult<()> {
        log_webview_start("WebViewInstance::type_text", &format!("selector={}, text={}", selector, text));
        let script = format!(
    ...
    pub fn snapshot(&self, format: &str) -> Result<String, String> {
        ...
        if let Some(controller) = &self.controller {
            unsafe {
                let webview = controller.CoreWebView2().map_err(|e| format!("CoreWebView2 error: {:?}", e))?;
                let request_id = Uuid::new_v4().to_string();

                log_webview_debug("WebViewInstance::snapshot", &format!("Executing script, request_id={}", request_id));
                webview.ExecuteScript(
                    &HSTRING::from(script),
                    &ICoreWebView2ExecuteScriptCompletedHandler::from(ExecuteScriptHandler {
                        request_id: request_id.clone(),
                ...
                // Wait for result (poll-based)
                let start = std::time::Instant::now();
                loop {
                    if let Some(result) = PENDING_SCRIPT_RESULTS.with(|map| {
                        map.borrow_mut().remove(&request_id)
                    }) {
                        log_webview_success("WebViewInstance::snapshot", Some(start.elapsed().as_millis()));
                        return result;
                    }

                    if start.elapsed() > std::time::Duration::from_secs(5) {
                        log_webview_error("WebViewInstance::snapshot", "Timeout after 5s");
                        return Err("Snapshot timeout".to_string());
                    }
    ...
    pub fn get_cookies(&self) -> WinResult<Vec<CookieInfo>> {
        ...
        if let Some(controller) = &self.controller {
            unsafe {
                ...
                log_webview_debug("WebViewInstance::get_cookies", &format!("Executing script, request_id={}", request_id));
                webview.ExecuteScript(
                    &HSTRING::from(script),
                    &ICoreWebView2ExecuteScriptCompletedHandler::from(ExecuteScriptHandler {
                        request_id: request_id.clone(),
                ...
                loop {
                    if let Some(result) = PENDING_SCRIPT_RESULTS.with(|map| {
                        ...
                        log_webview_success("WebViewInstance::get_cookies", Some(start.elapsed().as_millis()));
                        let json_str = match result {
                            Ok(s) => s,
                            Err(e) => {
                                return Err(Error::new(
                                    windows::core::HRESULT(0x80070057u32 as i32),
                                    HSTRING::from(format!("Script error: {:?}", e)),
                                ));
                            }
                        };
                        let cookies: Vec<CookieInfo> = match serde_json::from_str(&json_str) {
                            Ok(c) => c,
                            Err(e) => {
                                return Err(Error::new(
                                    windows::core::HRESULT(0x80070057u32 as i32),
                    ...
                    if start.elapsed() > std::time::Duration::from_secs(5) {
                        log_webview_error("WebViewInstance::get_cookies", "Timeout after 5s");
                        return Err(Error::new(
                            windows::core::HRESULT(0x80070057u32 as i32),
                            HSTRING::from("Get cookies timeout"),
                        ));
                    }
    ...
    pub fn set_cookie(&self, name: &str, value: &str, domain: &str) -> Result<(), String> {
        log_webview_start("WebViewInstance::set_cookie", &format!("name={}, value={}, domain={}", name, value, domain));
        let script = format!(
